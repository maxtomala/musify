'use strict'

var express =require('express')
var SongController = require ('../2-controller/song');
var api = express.Router(); //hace funciones get,post,put etc
var md_auth= require('../middlewwares/authenticated')

var multipart = require('connect-multiparty');
var md_upload = multipart({uploadDir:'./uploads/song'});

api.get('/song/:id', md_auth.ensureAuth, SongController.getSong)// crea ruta,metodo,cargar el metodo del controlador
api.post('/song', md_auth.ensureAuth, SongController.saveSong)
api.get('/songs/:album?', md_auth.ensureAuth, SongController.getSongs)
api.put('/song/:id', md_auth.ensureAuth, SongController.updateSong)
api.delete('/song/:id', md_auth.ensureAuth, SongController.deleteSong)
api.post('/upload-file-song/:id',[md_auth.ensureAuth, md_upload],SongController.uploadFile);
api.get('/get-song-file/:songFile', SongController.getSongFile);

//exportar el metodo del api
module.exports = api;